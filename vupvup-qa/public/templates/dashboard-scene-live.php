...
/** @var object $scene, WP_Post $event, int $event_id, int $scene_id, string $event_status, string $qr_url, string $landing */
?>
<!DOCTYPE html>
...
<head>
    ...
    <title><?php echo esc_html( $scene->name ); ?> — VupVup Dashboard</title>
    <?php wp_head(); ?>
</head>
...
<div class="vupvup-dash-wrap">
    ...
    <div class="vupvup-dash-header vupvup-event-header">
        <div>
            ...
            <h1><?php echo esc_html( $scene->name ); ?></h1>
            <p class="vupvup-sub-heading"><?php echo esc_html( $event->post_title ); ?></p>
        </div>
        <div class="vupvup-event-controls">
            <span id="vupvup-status-badge"
                  class="vupvup-badge vupvup-status-<?php echo esc_attr( $event_status ); ?>">
                <?php
                $labels = [ 'draft' => 'Kladde', 'active' => 'Live', 'closed' => 'Afsluttet' ];
                echo esc_html( $labels[ $event_status ] ?? $event_status );
                ?>
            </span>
        </div>
    </div>
